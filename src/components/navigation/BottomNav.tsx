...
// from this same 4rem figure so the two line up (see that file's own
// comment).
//
// 5-column grid: the middle column is a plain empty spacer, not a tab -
// it exists purely so the 4 real tabs split evenly (2 + 2) around the
// center action button, which is still rendered/owned entirely by
// ShoppingList.tsx (see FloatingAddButton.tsx) and just visually sits
// in this gap. No navigation/routing change - same 4 destinations,
// same order.
  ...
  return (
    ...
    >
      <div className="max-w-md sm:max-w-lg md:max-w-2xl mx-auto grid grid-cols-5 h-16">
        ...
        {/* Spacer for the center action button (FloatingAddButton),
            which overlaps this gap from above - see that component. */}
        <div aria-hidden="true" />

        {TABS.slice(2).map(({ to, label, Icon, ActiveIcon, end }) => (
          <NavLink
            key={to}
            to={to}
            end={end}
