...
const CATEGORY_STYLES: Record<string, CategoryStyle> = {
  ...
  // Requested emoji is the meat cut (🥩), not fish - this category
  // covers meat and fish together, but 🥩 is what the spec asked for.
  ...
  // "מאפים" is what the current default-categories migration seeds
  // going forward; "מאפים ולחם"/"מאפייה" are kept as aliases so lists
  // seeded by the old signup trigger (before this pass) still get a
  // color instead of falling back to gray.
  'מאפים': { icon: '🍞', bg: 'bg-amber-50', text: 'text-amber-700', fill: 'bg-amber-500', strip: 'bg-amber-500' },
